...
3. **无冗余交互原则**  
   禁止对同一物品重复执行相同的可交互动作（如多次 `examine`），除非有明确证据表明状态可能已变。每个物品应记录已执行动作，避免无效操作。

4. **未搜索区域必查原则**  
   **任何 `visited = true` 但 `searched = false` 的区域，只要任务尚未完成，就必须被执行 `Query_Items`**。  
   - 区域一旦被访问（无论是初始区域还是通过 `move_to` 到达），即视为“可能包含目标”；  
   - 若跳过未搜索区域，将导致无法保证“未找到”的结论可信；  
   - 因此，**不允许存在 visited 但未 searched 的区域**，除非任务已在之前终止。

5. **拓扑探明最小完备原则**  
   为确保场景认知完整，必须满足：  
   - **整个任务过程中至少执行一次 `Query_Regions`**；  
   - **每次成功执行 `move_to` 后，必须立即调用 `Query_Regions`**。  
   由此逐步构建完整区域邻接图，支撑“未搜索区域必查”的判断。

6. **彻底无解需透明报告原则**  
   仅当以下条件全部满足时，方可报告“未找到”：  
   - 所有通过 `Query_Regions` 发现的区域均已 **visited** 且 **searched**；  
   - 无任何 visited 但未 searched 的区域残留；  
   - 满足拓扑探明最小完备原则；  
   - 所有物品均已按原则2–3处理；  
   - 当前信息仍不足以完成任务。  
   报告须包含区域状态、物品列表、动作摘要及探索轨迹。
</Task>
