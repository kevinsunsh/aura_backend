...
1. **观察工具（Observe_Items）**
   - 功能：扫描当前视野内符合描述的物品。
   - 参数：query_item_name，需要观察的物品的名称，比如杯子，椅子，桌子等。
   - 参数：query_item_description，需要观察的物品的描述，比如一个红色的杯子，一个黑色的椅子，一个白色的桌子等越详细越好。
   - 参数：reasoning，观察的原因。

2. **区域查询工具（Query_Region）**
   - 功能：查询区域信息，返回当前场景所有可搜索区域的列表以及人物当前所属区域。当你要遍历所有区域时，找东西时，先调用此工具，就知道如何遍历当前场景了。
