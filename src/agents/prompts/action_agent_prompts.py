...
2. **执行工具（Execute_Action）**  
   - 功能：执行动作，动作里包含移动相关的可以扩大搜索范围的动作，也包含详细检查物品的动作，可以获得物品的详细信息。
...
请执行以下逻辑：
明白了。以下是进一步优化后的 prompt，明确引入了**有目的、有上下文感知的移动策略**，强调在选择移动目标时应基于当前搜索任务语义和已观察环境进行智能决策，而非随机或盲目移动：

---

请按以下逻辑执行搜索任务：
1. **判断已有信息是否充分**：首先检查 *Search Steps History* 中是否已包含完成当前 *Search Task* 所需的全部信息。  
   - 若已包含，则**立即调用 Report 工具**，返回搜索结果及后续搜索建议。
2. **若信息不足，优先在当前区域进行智能局部探索**：  
   - **优先调用 Observe_Items 工具**，获取当前视野（有限范围内）的物品信息。  
   - 由于 **Observe_Items 仅能观察一定范围内的物品**，需结合使用 **Execute_Action 工具** 移动至附近物品旁以扩展感知。  
   - **在选择移动目标时，应基于当前搜索任务的语义目标与环境中物品的关联性进行智能决策**：优先靠近那些在类别、功能、空间布局或上下文中更可能与目标任务相关的物品，以提高发现目标或获取有用线索的效率。避免无目的或纯随机移动。
3. **若当前区域信息仍不足以完成任务，扩展至新区域**：  
   - **优先调用 Query_Region 工具**，获取周边可探索区域的信息。  
   - 使用 **Execute_Action 工具** 移动至一个新区域。  
   - **进入新区域后，重新应用第 2 步的智能局部探索逻辑**：即根据任务目标，动态评估区域内物品的相关性，并优先靠近高潜力目标以调用 **Observe_Items**，最大化信息获取效率。
4. **一旦获得足够信息，立即终止搜索**：  
   - 若在任意区域（包括初始或新进入的区域）中，观察结果已足以完成搜索任务，则**立即调用 Report 工具**，返回结果及后续建议。
5. **若穷尽合理搜索区域仍无结果，终止并说明情况**：  
   - 若已系统性地探索所有合理可达区域（包括各区域内的智能局部探索），且仍未找到目标，则**调用 Report 工具**，明确说明“未找到”，并列出已搜索的区域及覆盖范围。
</Task>
