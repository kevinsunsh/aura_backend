...
class MemoryConfig(ConfigBase):
    ...
    # 回忆相关配置
    recall_max_memory_num: int = 5
    """回忆时最多获取的记忆数量"""

    recall_max_memory_length: int = 3
    """回忆时每条记忆的最大长度"""

    recall_max_depth: int = 2
    """回忆时的记忆图深度"""

    recall_fast_retrieval: bool = False
    """回忆时是否使用快速检索"""

    recall_confidence_threshold: float = 0.3
    """回忆时的置信度阈值"""

    # 记住相关配置
    memorize_min_content_length: int = 10
    """记住时的最小内容长度"""

    memorize_max_content_length: int = 2000
    """记住时的最大内容长度"""

    memorize_content_types: list[str] = field(default_factory=lambda: [
        "question", "acknowledgment", "explanation", "suggestion", "conversation_summary", "general"
    ])
    """记住时的内容类型列表"""

    memorize_confidence_threshold: float = 0.5
    """记住时的置信度阈值"""

    # 记忆构建相关配置
    build_memory_auto_trigger: bool = True
    """是否自动触发记忆构建"""

    build_memory_trigger_interval: int = 600
    """记忆构建触发间隔（秒）"""
